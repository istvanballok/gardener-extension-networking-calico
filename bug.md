...
Specifically, the bird exporter is disabled by default, but its scrape configuration is nevertheless deployed unconditionally. During a gardener/gardener PR validation, the respective empty scrape pool would trigger a health check finding. As a mitigation, an exception was added to ignore the empty scrape pool for the `shoot-calico-bird` scrape job. The Prometheus health checks are controlled via a feature gate: they are only executed during PR validation and are not enabled in real landscapes to allow for collecting and resolving findings like this one, without time pressure.
...
1. Create a shoot cluster without enabling the bird exporter (default configuration, the `.spec.networking.providerConfig.birdExporter.enabled` property of the `Shoot` resource is not set to `true`)
...
We prepared a draft PR that adds Helm template conditions to deploy each scrape configuration only when its corresponding component is enabled. While this prevents the issue in new shoot control plane deployments, we are not sure how to address the following scenarios:
...
The draft PR contains the attempted fix, but a more comprehensive solution is needed that includes:
...
We are opening this issue to explore a more comprehensive solution or collect feedback on possible approaches.
